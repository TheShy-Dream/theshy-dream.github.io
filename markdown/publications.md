---
layout: page
title: "Publications"
permalink: /publications/
---

# 📚 Publications
---

1. **Chen J**, Huang C*, Huang Q, Tu Y, Zheng Z, Huang X  
   **Test-Time Reinforcement Learning for Flow Matching**  
   International Conference on Machine Learning (ICML), 2026  
   🌐 [Project Page](../flow-ttrl/) | 🔗 [PDF](../assets/Flow-TTRL.pdf)  
   <span style="background-color:#FF6347; color:#FFFFFF; padding:2px 5px; border-radius:4px;">CCF-A</span>

2. **Chen J**, Zeng H, Huang C*, Huang Q, Li Z*, Huang X  
   **Context-Aware Latent Space Mediation for Inference-Time Unbiased Semantic Alignment in Text-to-Image Models**  
   Expert Systems with Applications, 2026: 133019  
   🔗 [PDF](../assets/CALM.pdf) | 🌐 [DOI](https://doi.org/10.1016/j.eswa.2026.133019)  
   <span style="background-color:#B0C4DE; color:#000000; padding:2px 5px; border-radius:4px;">SCI, IF: 7.5, Q1, CCF-C</span>

3. **Chen J**, Zhong Y, Huang Q, Huang C, Jiang F, Huang X  
   **UCMIB-PNS: Balancing Sufficiency and Necessity with Probabilistic Causality and Cross-Modal Uncertainty in Multimodal Sentiment Analysis**  
   IEEE Transactions on Affective Computing  
   🔗 [PDF](../assets/UCMIB-PNS.pdf) | 📄 [Supplement](../assets/UCMIB-PNS-SUPPLEMENT.pdf) | 🌐 [DOI](https://ieeexplore.ieee.org/abstract/document/11153045)  
   <span style="background-color:#FFD700; color:#000000; padding:2px 5px; border-radius:4px;">SCI, IF: 9.6, Q1 TOP, CCF-B</span>

4. Huang C, **Chen J**, Huang Q*, Wang S, Tu Y, Huang X  
   **AtCAF: Attention-based causality-aware fusion network for multimodal sentiment analysis**  
   Information Fusion, 2025, 114: 102725  
   🔗 [PDF](../assets/atcaf.pdf) | 🌐 [DOI](https://www.sciencedirect.com/science/article/abs/pii/S1566253524005037)  
   <span style="background-color:#90EE90; color:#000000; padding:2px 5px; border-radius:4px;">SCI, IF: 14.7, Q1 TOP, CAAI A</span>

5. **Chen J**, Huang Q*, Huang C*, Huang X  
   **Actual Cause Guided Adaptive Gradient Scaling for Balanced Multimodal Sentiment Analysis**  
   ACM Transactions on Multimedia Computing, Communications, and Applications (TOMM), 2025, 21(6): 1-24  
   🔗 [PDF](../assets/AGS-SMOE.pdf) | 🌐 [DOI](https://dl.acm.org/doi/10.1145/3736415)  
   <span style="background-color:#87CEEB; color:#000000; padding:2px 5px; border-radius:4px;">SCI, IF: 5.2, CCF-B</span>

6. Huang Q*, **Chen J**  
   **Enhancing academic performance prediction with temporal graph networks for massive open online courses**  
   Journal of Big Data, 2024, 11(1): 52  
   🔗 [PDF](../assets/Enhancing.pdf) | 🌐 [DOI](https://link.springer.com/article/10.1186/s40537-024-00918-5)  
   <span style="background-color:#FFA07A; color:#000000; padding:2px 5px; border-radius:4px;">SCI, IF: 8.6, Q2</span>

7. Tu Y, **Chen J**, Huang C*  
   **Empowering personalized learning with generative artificial intelligence: Mechanisms, challenges and pathways**  
   Frontiers of Digital Education, 2025, 2(2): 1-18  
   🌐 [DOI](https://link.springer.com/article/10.1007/s44366-025-0056-9)

8. Huang Q*, **Chen J**, Huang C, Wang Y, Huang X  
   **Text-centered cross-sample fusion network for multimodal sentiment analysis**  
   Multimedia Systems, 2024, 30(4): 228  
   🔗 [PDF](../assets/tecafn.pdf) | 🌐 [DOI](https://link.springer.com/article/10.1007/s00530-024-01421-w)  
   <span style="background-color:#DDA0DD; color:#000000; padding:2px 5px; border-radius:4px;">SCI, IF: 3.5, Q3</span>

9. Huang C*, Lin Z, Huang Q, Huang X, Jiang F*, **Chen J**  
   **H2CAN: heterogeneous hypergraph attention network with counterfactual learning for multimodal sentiment analysis**  
   Complex & Intelligent Systems, 2025, 11(4): 196  
   🔗 [PDF](../assets/H2CAN.pdf) | 🌐 [DOI](https://link.springer.com/content/pdf/10.1007/s40747-025-01806-y.pdf)  
   <span style="background-color:#AFEEEE; color:#000000; padding:2px 5px; border-radius:4px;">SCI, IF: 5.0, Q3</span>

10. Huang Q*, **Chen J**  
    **xLSTM-FER: Enhancing Student Expression Recognition with Extended Vision Long Short-Term Memory Network**  
    Asia-Pacific Web (APWeb) & WAIM 2024, 249-259  
    🔗 [PDF](../assets/xlstmfer.pdf) | 🌐 [DOI](https://link.springer.com/chapter/10.1007/978-981-96-0055-7_21)  
    <span style="background-color:#FFB6C1; color:#000000; padding:2px 5px; border-radius:4px;">EI</span>

**Note:** * indicates corresponding author